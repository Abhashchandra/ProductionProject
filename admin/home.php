...
<?php 
  ...
  $con = mysqli_connect('localhost','root','','expense_budget_db');
   ...
   if ($lcheck->num_rows == 1 ) {
    ...
    $sql = mysqli_query($con,"SELECT * from running_balance where balance_type = '2' and user_id = '$userid' and amount > '$limit' order by id DESC limit 5");
    while($row = mysqli_fetch_array($sql)){
      ...
      Your Expense is increasing on <?php echo $catname; ?> by Rs <?php echo $disamount; ?> 
    </div>
          <?php 
            ...
            <div class="info-box">
              ...
              <div class="info-box-content">
                ...
                <span class="info-box-number text-right">
                  <?php 
                    $cur_bul = $conn->query("SELECT sum(balance) as total FROM `categories` where status = 1 and userid = '$userid' ")->fetch_assoc()['total'];
          ...
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box mb-3">
              ...
              <div class="info-box-content">
                ...
                <span class="info-box-number text-right">
                  <?php 
                    $today_budget = $conn->query("SELECT sum(amount) as total FROM `running_balance` where category_id in (SELECT id FROM categories where status =1) and user_id = '$userid' and date(date_created) = '".(date("Y-m-d"))."' and balance_type = 1 ")->fetch_assoc()['total'];
          ...
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box mb-3">
              ...
              <div class="info-box-content">
                ...
                <span class="info-box-number text-right">
                <?php 
                    $today_expense = $conn->query("SELECT sum(amount) as total FROM `running_balance` where category_id in (SELECT id FROM categories where status =1) and user_id ='$userid' and date(date_created) = '".(date("Y-m-d"))."' and balance_type = 2 ")->fetch_assoc()['total'];
...
<div class="row row-cols-4 row-cols-sm-1 row-cols-md-4 row-cols-lg-4">
  <?php 
  $categories = $conn->query("SELECT * FROM `categories` where status = 1 and userid ='$userid' order by `category` asc ");
