            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    $userid = $_settings->userdata('id');
                        $qry = $conn->query("SELECT r.*,c.category,c.balance from `running_balance` r inner join `categories` c on r.category_id = c.id where c.status=1 and userid = '$userid' and r.balance_type = 2 and date(r.date_created) between '{$date_start}' and '{$date_end}' order by unix_timestamp(r.date_created) asc");
